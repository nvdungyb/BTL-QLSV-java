...
//        String maSv = "B21DCCN001";
        String sql ="SELECT l.id_sv, c.*, m.ten_mh FROM monhoc m, mh_lich l, course c " +
                     "WHERE (l.id_sv = '"+maSv+"') AND (m.id_mh = c.id_mh) AND (c.course_id = l.course_id);";

        try (Connection conn = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3306/demo1", "root", "2311");
             Statement state = conn.createStatement();
             ResultSet resultSet = state.executeQuery(sql)) {
//            if(!resultSet.next()){
//                System.out.println("Không có thông tin ?????, looic ở chỗ statement");
//            }
//            else{
